...
@author: mitch
"""
from core import state_abrv, catalog_name
import csv
import time
import requests
import sys
...
def WikiData_Search(town_list, town_catalog):
    ...
    for i in range(len(wiki_list)):
        ...
        # As per WikiData policy, cannot exceed 60 requests per 60 seconds.
        time.sleep(1)
...
def catalog_generator(town_list):
    ...
    # Towns not found are written to error_towns.csv for manual corrections.
        ...
        newtown_list, latitude_list, longitude_list = WikiData_Search(
            town_list, town_catalog)
        ...
        temp_list = [list(e) for e in zip(
            newtown_list, latitude_list, longitude_list)]
    ...
    final_towns_list, latitude_list, longitude_list, variant_town_list = remove_error_towns(
        newtown_list, latitude_list, longitude_list, towncatalog)

    final_list = [list(d) for d in zip(
        final_towns_list, latitude_list, longitude_list)]
...
def remove_error_towns(town_list, lat_list, long_list, towncatalog):
    ...
    error_towns_errorcheck(errortowns)
                ...
                if not errortowns[index][3]:
                    ...
                    try:
                        ...
                        if variant_town in town_catalog:
                            print("Found new name at index " + str(index) +
                                  " in local database: " + str(variant_town))
                            catalog_index = town_catalog.index(variant_town)
                            latitude = catalog_lat[catalog_index]
                            longitude = catalog_long[catalog_index]
                        else:
                            print("searching new name at index " +
                                  str(index) + " on Wikidata: " + str(variant_town))
                        ...
                        print("The modified town name " + variant_town +
                              " does not match any WikiData towns. Rerun when changed.")
                        sys.exit(1)
            else:
                print(str(
                    town_list[i]) + " is missing from error_towns_corrections.csv. Complete entry and rerun script.")
                sys.exit(1)
...
def error_towns_errorcheck(errortowns):
    ...
    for i in range(len(errortowns)):
        ...
        if condition_list != [True, True, False] and condition_list != [False, False, True]:
            print(
                "error_towns_corrections.csv has improper formating at index: " + str(i))
            sys.exit(1)
